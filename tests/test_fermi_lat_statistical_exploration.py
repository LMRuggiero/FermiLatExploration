...
test_df = Source4FGLData.from_4fgl_file(data_path)
...
class TestSource4FGLData(unittest.TestCase):
    ...
    def test_save_or_show_plot(self):
        """
        (static method) Check if the method save the plot image in the right output folder
        """
        x = range(10)
        y = range(5, 15)
        plt.figure()
        ...
        Source4FGLData.save_or_show_plot(True, test_plot_name)
    ...
    def test_from_astropy_table(self):
        """
        (class method) Check if the method correctly convert an astropy table in a Source4FGLData object
        """
